...
def run_trial_from_grid(
    grid_params, trial_index, seeds, learning_steps=None, cycles=None
):
    ...
    run_subdir = (
        f"trial_{trial_index}_d{discrete_lr:.0e}_c{continuous_lr:.0e}_u{update_ratio}"
    )
...
def save_grid_timing_summary(total_duration, trial_durations, num_trials, num_seeds):
    ...
    timing_data = {
        ...
        "avg_job_duration_seconds": np.mean(trial_durations)
        if trial_durations
        else None,
        "avg_job_duration": format_duration(np.mean(trial_durations))
        if trial_durations
        else None,
        "min_job_duration_seconds": np.min(trial_durations)
        if trial_durations
        else None,
        "max_job_duration_seconds": np.max(trial_durations)
        if trial_durations
        else None,
...
def run_grid_search(client, learning_steps=None, cycles=None, seeds=None):
    ...
    print(
        f"[INFO] Grid search started at {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}"
    )
    if run_in_parallel:
        print(
            f"[INFO] Running in FULL PARALLEL mode: {len(GRID_PARAMS)} trials × {len(seeds)} seeds = {len(GRID_PARAMS) * len(seeds)} parallel jobs"
        )
    else:
        print(
            f"[INFO] Running in SEQUENTIAL mode (debug): {len(GRID_PARAMS)} trials × {len(seeds)} seeds"
        )

    if ROTATE_SEEDS_PER_TRIALS:
        print(
            "[INFO] ROTATE_SEEDS_PER_TRIALS=True: Each trial will use different seeds (sequential chunks)"
        )
    ...
    for grid_idx, grid_params in enumerate(GRID_PARAMS):
        ...
        if ROTATE_SEEDS_PER_TRIALS:
            num_seeds = len(seeds)
            trial_seeds = [
                BASE_SEED + grid_idx * num_seeds + i for i in range(num_seeds)
            ]
        else:
            trial_seeds = seeds
    ...
    if run_in_parallel:
        ...
        for job_info in all_jobs:
            ...
            print(
                f"[INFO] Trial {trial_index}, seed {seed} completed: mean_reward = {result['mean_reward'][0]:.4f} ({format_duration(job_duration)})"
            )
        ...
        for trial_index, data in trial_results.items():
            ...
            client.complete_trial(
                trial_index=trial_index, raw_data={"mean_reward": (mean_reward, sem)}
            )
            ...
            print(
                f"[INFO] Trial {trial_index} complete: mean_reward = {mean_reward:.4f} ± {sem:.4f}"
            )
