...
def analyze_subsets(merged_df: pd.DataFrame, max_n: int = None):
    ...
    for n in range(2, max_n + 1):
        ...
        if len(groups) >= 2 and len(groups[0]) >= 2:
            ...
            results.append(
                {"n": n, "F": f_stat, "p_value": p_value, "significant": p_value < 0.05}
            )
...
def print_summary(merged_df: pd.DataFrame):
    ...
    print("\n--- Per-Cell Statistics ---")
    cell_stats = (
        merged_df.groupby(["discrete_lr", "continuous_lr"])
        .agg({"mean_reward": ["mean", "std", "count"]})
        .round(4)
    )
    print(cell_stats)
...
def create_visualizations(merged_df: pd.DataFrame, output_dir: str = "."):
    ...
    pivot = latest_data.pivot(
        index="continuous_lr", columns="discrete_lr", values="mean_reward"
    )
    ...
    std_data = (
        latest_data.groupby(["continuous_lr", "discrete_lr"])["mean_reward"]
        .std()
        .unstack()
    )
    sns.heatmap(
    ...
    if anova_results is not None and len(anova_results) > 0:
        ...
        ax.axhline(
            y=0.05, color="r", linestyle="--", label="p = 0.05 significance threshold"
        )
        ax.fill_between(
            anova_results["n"],
            0,
...
def plot_bo_progress(run_dir):
    ...
    if not history_path.exists() and not wip_path.exists():
        print(
            f"[ERROR] No best_scores_history.csv or wip-client.csv found in {run_dir}"
        )
        return
    ...
    ax.plot(
        best_so_far_x, best_so_far_y, "g--", linewidth=2, label="Best So Far", zorder=1
    )
    ...
    if complete_x:
        ax.scatter(
            ...
            s=100,
            marker="o",
            label=f"Complete ({len(complete_x)})",
            zorder=3,
        )
    ...
    if inprogress_x:
        ax.scatter(
            ...
            marker="x",
            linewidths=2,
            label=f"In Progress ({len(inprogress_x)})",
            zorder=3,
        )
    ...
    for trial_idx in sorted_trials:
        ...
        ax.annotate(
            label,
            (trial_idx, data["mean_reward"] + offset),
            fontsize=8,
            ha="center",
            va="bottom" if data["complete"] else "top",
        )
...
def find_runs(run_type: str = None, state: str = None) -> list:
    """Find runs matching criteria in new structure."""
    runs_root = Path("runs")
    ...
    results = []
    for type_dir in runs_root.iterdir():
        if not type_dir.is_dir():
            continue
        if run_type and type_dir.name != run_type:
            continue
        for scale_dir in type_dir.iterdir():
            if not scale_dir.is_dir():
                continue
            for state_dir in scale_dir.iterdir():
                if not state_dir.is_dir():
                    continue
                if state and state_dir.name != state:
                    continue
                for run in state_dir.iterdir():
                    if run.is_dir():
...
def main():
    ...
    parser.add_argument(
        "run_dirs", nargs="*", help="Run directories to merge (or glob pattern)"
    )
    parser.add_argument(
        "--bo-progress",
        metavar="DIR",
        help="Show BO progress plot (specify run directory)",
    )
    parser.add_argument(
        "--output", "-o", default="merged_grid_results.csv", help="Output filename"
    )
    ...
    parser.add_argument(
        ...
        nargs="*",
        metavar=("TYPE", "STATE"),
        help="Find runs: grid|bayesian [complete|incomplete]",
    )
    ...
    # Find runs mode
    if args.find:
        run_type = args.find[0] if len(args.find) > 0 else None
        state = args.find[1] if len(args.find) > 1 else None
        runs = find_runs(run_type, state)
        if runs:
            print("\n".join(runs))
        else:
            print("No runs found")
    ...
    if args.visualize:
        output_dir = (
            Path(args.output).parent if Path(args.output).parent != Path(".") else "."
        )
