...
name="${first_alg}-${second_alg}-${env}-cycles${CYCLES}-job${SLURM_ARRAY_JOB_ID}"
...
which poetry
echo ==RUN==
echo >python3 train.py hydra.job.name=$name algorithm=$alg environment=$env parameters.seeds=[$((SLURM_ARRAY_TASK_ID + SEED_OFFSET))] parameters.cycles=$CYCLES
python3 /storage/users/tollet/SARL/sarl/train.py hydra.job.name=$name algorithm=$alg environment=$env parameters.seeds=[$((SLURM_ARRAY_TASK_ID + SEED_OFFSET))] parameters.cycles=$CYCLES
