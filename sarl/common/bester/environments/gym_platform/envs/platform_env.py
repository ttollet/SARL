...
class PlatformEnv(gym.Env):
    ...
    # cannot use rgb_array at the moment due to frame skip between actions
    ...
    def __init__(self):
        ...
        self.platform2 = Platform(
            Constants.GAP1 + self.platform1.size[0], Constants.HEIGHT2, Constants.WIDTH2)
    ...
    def step(self, action):
        ...
        # print(act,param)
        param = np.clip(
            param, Constants.PARAMETERS_MIN[act_index], Constants.PARAMETERS_MAX[act_index])
        ...
        while running:
            ...
            truncated = terminal  # TODO: T Separate truncation from termination?
    ...
    def reset(self, seed=None, options=None):
        ...
        self.states = []
        self.render_states = []
        # 0 represents number of steps since last obs?
        obs = (self.get_state(), 0)
        return obs, {}  # Edited to return obs instead of self.get_state() directly since gym was complaining
    ...
    def _lower_bound(self):
        ...
        lower = min(
            self.platform1.position[1], self.platform2.position[1], self.platform3.position[1])
        return lower
    ...
    def _terminal_check(self, reward=0.0):
        ...
        end_episode = self.player.position[1] < self._lower_bound(
    ...
    def _initialse_window(self):
        ...
        if self.window is None:
            ...
            self.platform_sprite = pygame.image.load(
                PLATFORM_PATH).convert_alpha()
    ...
    def save_render_states(self, dir, prefix, index=0):
        ...
        for s in self.render_states:
            ...
            pygame.image.save(self.window, os.path.join(
                dir, prefix+"_"+str("{:04d}".format(index))+".bmp"))
            index += 1
        return index
...
class Enemy:
    ...
    def update(self, dt):
        ...
        right = self.platform.position[0] + \
            self.platform.size[0] - self.size[0]
        ...
        self.position[0] = np.clip(
            self.position[0], self.platform.position[0], right)
...
class Player:
    ...
    def update(self, dt):
        ...
        self.position[0] = np.clip(
            self.position[0], 0.0, Constants.TOTAL_WIDTH)
    ...
    def accelerate(self, accel, dt=Constants.DT):
        ...
        accel = np.clip(accel, (-Constants.MAX_DDX, -Constants.MAX_DDY),
                        (Constants.MAX_DDX, Constants.MAX_DDY))
        self.velocity += accel * dt
        self.velocity[0] -= abs(self.np_random.normal(0.0,
                                Constants.PLAYER_NOISE * dt))
