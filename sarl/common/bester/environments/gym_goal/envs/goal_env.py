...
class GoalEnv(gym.Env):
    ...
    def step(self, action):
        ...
        param = np.clip(
            param, PARAMETERS_MIN[act_index], PARAMETERS_MAX[act_index])
        ...
        while run:
            ...
            if run:
                ...
                if act == DRIBBLE:
                    ...
                    run = False
        state = self.get_state()
        return (state, steps), reward, end_episode, False, {}  # TODO: Differentiate between termination and truncation
    ...
    def reset(self, seed=None, options=None):
        ...
        initial_player = np.array(
            (0, self.np_random.uniform(-PITCH_WIDTH / 2, PITCH_WIDTH / 2)))
        ...
        initial_ball = initial_player + \
            (KICKABLE - MACHINE_EPSILON) * angle_position(angle)
        ...
        return (self.get_state(), 0), {}  # Amended for gymnasium
    ...
    def _keeper_target(self, ball):
        ...
        if ball[0] < PITCH_LENGTH / 2 - GOAL_AREA_LENGTH:
            ...
            if ball[1] < -GOAL_AREA_WIDTH / 2:
                ...
                xval = (GOAL_AREA_WIDTH / 2 - yint) / grad
        xval = bound(xval, PITCH_LENGTH / 2 -
                     GOAL_AREA_LENGTH, PITCH_LENGTH / 2)
        yval = bound(grad * xval + yint, -GOAL_AREA_WIDTH /
                     2, GOAL_AREA_WIDTH / 2)
    ...
    def __draw_internal_state(self, internal_state, fade=False):
        ...
        self.__draw_horizontal(
            width / 2 - self.__visualiser_scale(GOAL_WIDTH / 2), length, depth)
        self.__draw_horizontal(
            width / 2 + self.__visualiser_scale(GOAL_WIDTH / 2), length, depth)
    ...
    def __draw_box(self, area_width, area_length):
        ...
        lower_corner = self.__visualiser_scale(
            PITCH_WIDTH / 2 - area_width / 2)
        ...
        self.__draw_horizontal(
            lower_corner, line, self.__visualiser_scale(PITCH_LENGTH / 2))
        self.__draw_horizontal(
            upper_corner, line, self.__visualiser_scale(PITCH_LENGTH / 2))
    ...
    def save_render_states(self, dir, prefix, index=0):
        ...
        for s in self.render_states:
            ...
            pygame.image.save(self.window, os.path.join(
                dir, prefix+"_"+str("{:04d}".format(index))+".jpeg"))
            index += 1
        return index
...
class Entity:
    ...
    def __init__(self, config):
        ...
        self.np_random: np.random.Generator = None  # overwritten by seed()
...
class Goalie(Player):
    ...
    def can_catch(self, ball):
        ...
        return self.np_random.random() <= CATCH_PROBABILITY and can_catch
