...
class SoccerParameterisedActionWrapper(gym.ActionWrapper):
    ...
    def action(self, action):
        ...
        """
        act = np.concatenate(
            (np.array([action[0],]), np.concatenate(action[1])))
        return act
...
class SoccerScaledParameterisedActionWrapper(gym.ActionWrapper):
    ...
    def action(self, action):
        ...
        p = action[0]
        if p == 0:
            action[1] = self.range[0][0] * \
                (action[1] + 1) / 2. + self.low[0][0]
            action[2] = self.range[0][1] * \
                (action[2] + 1) / 2. + self.low[0][1]
        elif p == 1:
            action[3] = self.range[1] * (action[3] + 1) / 2. + self.low[1]
        elif p == 2:
            action[4] = self.range[2][0] * \
                (action[4] + 1) / 2. + self.low[2][0]
            action[5] = self.range[2][1] * \
                (action[5] + 1) / 2. + self.low[2][1]
        else:
