from sarl.common.bester.agents.basis import ScaledBasis
import numpy as np
import sys
import os
import gymnasium as gym
import sarl.common.bester.environments.gym_goal
from sarl.common.bester.environments.gym_goal.envs.config import GOAL_WIDTH, PITCH_LENGTH, PITCH_WIDTH
        ...
        if depth == 14 or count == 2:
            ...
            if depth in [3, 4, 7, 8, 12, 13]:
                ...
                for j in range(FOURIER_DIM):
                    ...
                    self.__generate_coefficients(
                        coeffs, new_vector, depth + 1, count + (j > 0))
...
class GoalObservationWrapper(gym.ObservationWrapper):
    ...
    def __init__(self, env):
        ...
        newshape = (base_state.shape[0] +
                    ball_feats.shape[0] + keeper_feats.shape[0],)
    ...
    def keeper_projection(state):
        ...
        return np.clip(pos, -GOAL_WIDTH / 2, GOAL_WIDTH / 2)
    ...
    def observation(self, obs):
        ...
        state = np.concatenate(
            (state, self.ball_features(state), self.keeper_features(state)))
        return (state, steps)
