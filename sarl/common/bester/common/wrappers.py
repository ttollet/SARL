...
class ScaledStateWrapper(gym.ObservationWrapper):
    ...
    def __init__(self, env):
        ...
        if isinstance(obs, gym.spaces.Box):
            ...
            assert len(obs.spaces) == 2 and isinstance(
            ...
            raise Exception("Unsupported observation space type: %s" %
...
class ScaledParameterisedActionWrapper(gym.ActionWrapper):
    ...
    def __init__(self, env):
        ...
        self.high = [self.old_as.spaces[i].high for i in range(
            1, self.num_actions + 1)]
        self.low = [self.old_as.spaces[i].low for i in range(
            1, self.num_actions + 1)]
        self.range = [self.old_as.spaces[i].high -
                      self.old_as.spaces[i].low for i in range(1, self.num_actions + 1)]
        new_params = [  # parameters
            Box(-np.ones(self.old_as.spaces[i].low.shape), np.ones(
                self.old_as.spaces[i].high.shape), dtype=np.float32)
...
class QPAMDPScaledParameterisedActionWrapper(gym.ActionWrapper):
    ...
    def __init__(self, env):
        ...
        self.old_as = env.action_space
        self.num_actions = self.old_as.spaces[0].n
        self.high = [self.old_as.spaces[1]
                     [i].high for i in range(self.num_actions)]
        self.low = [self.old_as.spaces[1]
                    [i].low for i in range(self.num_actions)]
        self.range = [self.old_as.spaces[1][i].high -
                      self.old_as.spaces[1][i].low for i in range(self.num_actions)]
        new_params = [  # parameters
