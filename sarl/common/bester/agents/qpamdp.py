...
class QPAMDPAgent(Agent):
    ...
    def __init__(self, observation_space, action_space,
                 ...
                 discrete_agent=None,
                 norm_grad=False,
                 # list of variances per continuous action parameter
                 # (one entry per action, or 1 entry used across actions)
                 # defaults to 0.01, previously None TODO: Is there a better default?
                 variances=0.01,
                 seed=None,
                 phi0_func=None,
                 phi0_size=None,
        ...
        if self.parameter_obs_index is not None:
            ...
            if isinstance(self.parameter_obs_index[0], (list, np.ndarray)):
                if len(self.parameter_obs_index) == 1:
                    self.parameter_obs_index = np.tile(
                        self.parameter_obs_index, (self.num_actions, 1))
                else:
            ...
            for a in range(self.num_actions):
                ...
                if poly_basis is True:
                    self.basis.append(PolynomialBasis(
                        nvars, order=2, bias_unit=True))
                else:
        ...
        # self.parameter_weights = np.zeros((self.num_actions, self.num_basis_functions))  # TODO: Can improve init weights method?
    ...
    def learn(self, env, max_episodes=100000, max_steps_per_episode=None):
        ...
        self.discrete_agent.temperature = 0.0
        self.discrete_agent.epsilon = 0.0
        return info_per_episode  # Added for recording statistics
    ...
    def _set_parameters(self, parameters):
        ...
        for action in range(self.num_actions):
            ...
            self.parameter_weights[action] = parameters[index: index + rows
                                                        ].reshape(self.parameter_weights[action].shape)
            index += rows
    ...
    def _pad_action(self, act, param):
        ...
        action = [np.zeros(self.parameter_space.spaces[a].shape)
                  for a in range(self.num_actions)]
        action[act] = param
        action = (act, action)
        return action

    def _rollout(self, env, update_actions=False, max_steps=None, with_info=False):
        ...
        while not terminal and not (max_steps is not None and steps > max_steps):
            ...
            if update_actions:
                self.discrete_agent.step(
                    state, act, reward, new_state, new_act, terminal, time_steps)
            state = new_state
            states.append(state)
    ...
    def _action_update(self, state, action, reward, next_state, next_action, terminal, time_steps=1):
        self.discrete_agent.step(
            state, action[0], reward, next_state, next_action[0], terminal, time_steps)
