...
from sarl.common.bester.agents.memory.memory import Memory
from sarl.common.bester.agents.utils import soft_update_target_network, hard_update_target_network
from sarl.common.bester.agents.utils.noise import OrnsteinUhlenbeckActionNoise
    ...
    def __init__(self, state_size, action_size, action_parameter_size, hidden_layers=(100,), action_input_layer=0,
        ...
        for i in range(0, len(self.layers) - 1):
            nn.init.kaiming_normal_(
                self.layers[i].weight, nonlinearity=activation)
            nn.init.zeros_(self.layers[i].bias)
        if output_layer_init_std is not None:
            nn.init.normal_(self.layers[-1].weight,
                            mean=0., std=output_layer_init_std)
        # else:
...
class ParamActor(nn.Module):
    ...
    def __init__(self, state_size, action_size, action_parameter_size, hidden_layers, squashing_function=False,
        ...
        # unsupported, cannot get scaling right yet
        ...
        self.action_parameters_output_layer = nn.Linear(
            lastHiddenLayerSize, self.action_parameter_size)
...
class PDQNAgent(Agent):
    ...
    def __init__(self,
        ...
        self.action_max = torch.from_numpy(
            np.ones((self.num_actions,))).float().to(device)
        ...
        assert (weighted ^ average ^ random_weighted) or not (
            weighted or average or random_weighted)
        ...
        self.noise = OrnsteinUhlenbeckActionNoise(
            self.action_parameter_size, random_machine=self.np_random, mu=0., theta=0.15, sigma=0.0001)  # , theta=0.01, sigma=0.01)
        ...
        self.actor = actor_class(
            self.observation_space.shape[0], self.num_actions, self.action_parameter_size, **actor_kwargs).to(device)
        ...
        self.actor_param = actor_param_class(
            self.observation_space.shape[0], self.num_actions, self.action_parameter_size, **actor_param_kwargs).to(device)
    ...
    def act(self, state):
        with torch.no_grad():
            ...
            offset = np.array([self.action_parameter_sizes[i]
                              for i in range(action)], dtype=int).sum()
            ...
            action_parameters = all_action_parameters[offset:offset +
                                                      self.action_parameter_sizes[action]]
    ...
    def step(self, state, action, reward, next_state, next_action, terminal, time_steps=1):
        ...
        ), reward, next_state, np.concatenate(([next_action[0]], next_action[1])).ravel(), terminal=terminal)
    ...
    def _optimize_td_loss(self):
        ...
        with torch.no_grad():
            ...
            pred_Q_a = self.actor_target(
                next_states, pred_next_action_parameters)
        ...
        if self.clip_grad > 0:
            torch.nn.utils.clip_grad_norm_(
                self.actor_param.parameters(), self.clip_grad)

        self.actor_param_optimiser.step()

        soft_update_target_network(
            self.actor, self.actor_target, self.tau_actor)
        soft_update_target_network(
    ...
    def load_models(self, prefix):
        ...
        self.actor.load_state_dict(torch.load(
            prefix + '_actor.pt', map_location='cpu'))
        self.actor_param.load_state_dict(torch.load(
            prefix + '_actor_param.pt', map_location='cpu'))
